...
export default function AdminHofUploadPage() {
    ...
    const handleSubmit =  () => {
      ...
      BackEnd.post("hall-of-famers", formData, {}, {}, false, true)
      .then((resp) => {
  ...
  <Grid container direction="column" justifyContent="space-evenly" alignItems="center" spacing={4}>
      ...
      <Typography variant={"h6"} align={"center"} color={"primary"} className={classes.title}> Upload Hall of Fame File </Typography>
      ...
      <Button onClick={handleSubmit} color="primary" variant="contained" className={classes.margin}> Save</Button>
      
      {uploadComplete && (
        <Alert onClose={()=>setUploadComplete(false)} severity="success"> File Sucessfully Saved </Alert>
      )}
      {showError && (
        <Alert onClose={()=>setShowError(false)} severity="error">Error Uploading File.</Alert>
      )}
