...
export default function AdminForecastPage() {
  ...
  useEffect(() => {
    if (user && currentDate) {
      BackEnd.get(`entry/${user}/${currentDate}`).then((resp) => {
        if (resp?.status < 300) {
          setValues({
            high: resp.data.high,
            low: resp.data.low,
  ...
  }, [user, currentDate, users, forecastDates]);

  useEffect(() => {
    //  get users
    BackEnd.get(`user`).then((resp) => {
      if (resp?.status < 300) {
        setUsers(resp.data);
      }
    });
  ...
  const handleOnSelectUser = () => (event) => {
